...
x = np.loadtxt('/home/luke/Schreibtisch/x.txt')
y = np.loadtxt('/home/luke/Schreibtisch/y.txt')
...
#     y *= -1
    ...
    x, y = np.loadtxt('/home/luke/Schreibtisch/messwerte.txt', usecols = (1,0), unpack = True)
    
   
    
    
    _max, _min = peakdetect(y, x, 100, 0.30)
    #_max, _min = peakdetect_sine_locked(y, x, 9)
    #_max, _min = peakdetect_fft(y, x)
    xm = [p[0] for p in _max]
    ym = [p[1] for p in _max]
    xn = [p[0] for p in _min]
    yn = [p[1] for p in _min]
